...
* `platformOverridesSupportAttributeName` - deprecated since CEQ 3.33.1.ER1. Use `platformOverridesSupportAttributeNames` instead.
  If set, the value will effectively get added to `platformOverridesSupportAttributeNames` as the first element.
  Since cq-prod-maven-plugin 4.18.0
* `platformOverridesSupportAttributeNames` - a list of attribute names to put under `<extension>.metadata` in platform overrides files
  ...
  For 3.33.0+ versions: a single value `redhat-camel-support` for Camel Quarkus extensions and `["redhat-camel-support", "ibm-support"]` for Quarkus CXF extensions.
  See also https://github.com/l2x6/cq-maven-plugin/issues/717.
  Since cq-prod-maven-plugin 4.22.0
...
Helper script for automated testing of Camel Quarkus against Red Hat product builds (MRRC).

Key features:

* Auto-detects versions from MRRC repository
* Prepares BOM/POM adjustments for product builds
* Installs test-support modules
* Runs JVM and native integration tests
* Supports Jenkins CI integration
...
For complete usage, see the script's `--help` option or inline documentation.
