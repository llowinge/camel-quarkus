...
public class JmsRoutes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        fromF("%s:queue:testJmsTransferExchange?transferExchange=true"
                + (!componentScheme.getScheme().startsWith("sjms") ? "&objectMessageEnabled=true" : ""), componentScheme)
                .to("mock:transferExchangeResult");

        fromF("%s:queue:testJmsTransferException?transferException=true"
                + (!componentScheme.getScheme().startsWith("sjms") ? "&objectMessageEnabled=true" : ""), componentScheme)
